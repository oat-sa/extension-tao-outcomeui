define("taoOutcomeUi/controller/routes",[],function(){return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}}}}),define("taoOutcomeUi/controller/inspectResults",["jquery","i18n","module","helpers","layout/actions/binder","layout/section","uri","ui/feedback","ui/datatable"],function($,__,module,helpers,binder,section,uri,feedback){function loadResults($container,model,classUri){var params={classUri:classUri};$(".inspect-results-grid",$container).empty().data("ui.datatable",null).on("load.datatable",function(){$(".export-table",$container).off("click").removeClass("disabled").on("click",function(e){e.preventDefault(),section.create({id:"buildTableTab",name:__("Export Delivery Results"),url:helpers._url("index","ResultTable","taoOutcomeUi",params),contentBlock:!0}).show()
})}).datatable({url:helpers._url("getResults","Results","taoOutcomeUi",params),model:model,actions:{view:function(id){var action={binding:"load",url:helpers._url("viewResult","Results","taoOutcomeUi")};binder.exec(action,{uri:uri.encode(id),classUri:classUri}||this._resourceContext)},"delete":function(id){{var confirmBox=$(".preview-modal-feedback"),cancel=confirmBox.find(".cancel"),save=confirmBox.find(".save");confirmBox.find(".modal-close")}confirmBox.modal({width:500}),save.off("click").on("click",function(){$.ajax({url:helpers._url("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(id)},dataType:"json",success:function(response){response.deleted?(feedback().success(__("Result has been deleted")),loadResults($container,model,classUri)):feedback().error(__("Something went wrong ...")+"<br>"+response.error)
}}),confirmBox.modal("close")}),cancel.off("click").on("click",function(){confirmBox.modal("close")})}}})}var inspectResultController={start:function(){var conf=module.config(),$container=$("#inspect-result");loadResults($container,conf.model,conf.uri)}};return inspectResultController}),define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","helpers","module","ui/datatable","jquery.fileDownload"],function($,_,__,helpers,module){var resulTableController={start:function(){var conf=module.config(),$container=$(".result-table"),$filterField=$(".result-filter",$container),$tableContainer=$(".result-table-container",$container),filter=conf.filter||"lastSubmitted",classUri=conf.classUri||"",columns=[],groups={},buildGrid=function(url,action,done){$.ajax({url:url,dataType:"json",data:{filter:filter,classUri:classUri},type:"GET"}).done(function(response){response&&response.columns&&(columns="remove"===action?_.reject(columns,function(col){return _.find(response.columns,function(resCol){return _.isEqual(col,resCol)
