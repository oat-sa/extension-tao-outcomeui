define("taoOutcomeUi/controller/routes",[],function(){"use strict";return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}},ResultsMonitoring:{actions:{index:"controller/resultsMonitoring"}}}}),define("taoOutcomeUi/plugins/results/action/view",["i18n","layout/actions/binder","core/plugin","util/url"],function(__,binder,pluginFactory,urlHelper){"use strict";return pluginFactory({name:"view",init:function(){var resultsList=this.getHost(),action={binding:"load",url:urlHelper.route("viewResult","Results","taoOutcomeUi")};resultsList.addAction({id:"view",label:__("View"),
icon:"view",action:function(id){var context={id:id,classUri:resultsList.getClassUri()};binder.exec(action,context)}})}})}),define("taoOutcomeUi/plugins/results/action/delete",["jquery","i18n","uri","core/plugin","util/url","util/encode","ui/dialog/confirm","ui/feedback"],function($,__,uri,pluginFactory,urlHelper,encode,dialogConfirm,feedback){"use strict";return pluginFactory({name:"delete",init:function(){var resultsList=this.getHost();resultsList.addAction({id:"delete",label:__("Delete"),icon:"delete",action:function(id){dialogConfirm(__("Please confirm deletion"),function(){$.ajax({url:urlHelper.route("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(id)
...
define("taoItems/previewer/adapter/legacy",["util/url","taoItems/preview/preview"],function(urlHelper,preview){"use strict";return{name:"legacy",init:function(uri,state,config){return"object"==typeof uri&&uri.uri&&(uri=uri.uri),window.scrollTo(0,0),preview.init(urlHelper.route("forwardMe","ItemPreview","taoItems",{uri:uri}),state),preview.show(),preview}}}),define("taoItems/previewer/factory",["lodash","context","module","core/providerLoader","core/providerRegistry","util/url","taoItems/previewer/adapter/legacy"],function(_,context,module,providerLoaderFactory,providerRegistry,urlHelper,legacyPreviewer){"use strict";function previewerFactory(type,uri,state,config){
return config=_.defaults(config||{},module.config()),providerLoaderFactory().addList(config.previewers).load(context.bundle).then(function(providers){previewerFactory.registerProvider(legacyPreviewer.name,legacyPreviewer),_.forEach(providers,function(provider){previewerFactory.registerProvider(provider.name,provider)})}).then(function(){return previewerFactory.getProvider(type||legacyPreviewer.name)}).then(function(provider){return provider.init(uri,state,config)})}return providerRegistry(previewerFactory,function(provider){if(!_.isFunction(provider.init))throw new TypeError("The previewer provider MUST have a init() method");return!0})}),define("taoOutcomeUi/controller/viewResult",["module","jquery","i18n","util/url","layout/section","taoItems/previewer/factory","jquery.fileDownload"],function(module,$,__,urlHelper,section,previewerFactory){
"use strict";var viewResultController={start:function(){var conf=module.config(),$container=$("#view-result"),$resultFilterField=$(".result-filter",$container),$classFilterField=$('[name="class-filter"]',$container),classFilter=JSON.parse(conf.filterTypes)||[];$resultFilterField.select2({minimumResultsForSearch:-1}).select2("val",conf.filterSubmission||"all");for(var i in classFilter)$('[value="'+classFilter[i]+'"]').prop("checked","checked");$(".result-filter-btn",$container).click(function(e){classFilter=[""],$classFilterField.each(function(){$(this).prop("checked")&&classFilter.push($(this).val())}),section.loadContentBlock(urlHelper.route("viewResult","Results","taoOutcomeUi"),{
