...
self.trigger("error",err),taskQueue.pollAll()}):taskQueue.archive(task.id).then(function(){self.trigger("finished",result),taskQueue.pollAll()}).catch(function(err){self.trigger("error",err),taskQueue.pollAll()}):(message=__("<strong> %s </strong> has been moved to the background.",task.taskLabel),infoBox=feedback(null,{encodeHtml:!1,timeout:{info:8e3}}).info(message),taskQueue.trigger("taskcreated",{task:task,sourceDom:infoBox.getElement()}),self.trigger("enqueued",result)),loadingBar.stop()}).catch(function(err){taskQueue.pollAll(),self.trigger("error",err)
})):this.trigger("error","the taskQueue model is required to create a task")):this.trigger("error","the request url is required to create a task")},displayReport:function(report,title,result){var $reportContainer,self=this;if(this.config.taskReportContainer instanceof $)return $reportContainer=$(reportTpl({title:title})),this.config.taskReportContainer.html($reportContainer),reportFactory({actions:[{id:"continue",icon:"right",title:"continue",label:__("Continue")}]},report).on("action-continue",function(){self.trigger("continue",result)}).render($reportContainer.find(".report"))}};return function(component,config){
return _.assign(component,taskableComponent),component.off(".taskable").on("init.taskable",function(){_.defaults(this.config,config||{},defaultConfig)})}}),define("tpl!taoTaskQueue/component/button/tpl/treeButton",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",escapeExpression=this.escapeExpression;return buffer+='<div class="tree-button-component">\n    <a class="li-inner">\n        <span class="spinner-icon icon-loop spinning"></span>\n        <span class="glyph start-icon icon-',
(helper=helpers.icon)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.icon,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></span>\n        ',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"\n    </a>\n</div>"})}),define("css!taoTaskQueue/component/button/css/treeButton",[],function(){}),
define("taoTaskQueue/component/button/treeButton",["jquery","lodash","i18n","ui/feedback","ui/component","layout/loading-bar","taoTaskQueue/component/button/taskable","tpl!taoTaskQueue/component/button/tpl/treeButton","css!taoTaskQueue/component/button/css/treeButton"],function($,_,__,feedback,component,loadingBar,makeTaskable,buttonTpl){"use strict";var _defaults={icon:"property-advanced",label:"OK"},buttonApi={start:function(){return this.createTask(),this.setState("started",!0),this.trigger("start"),this},stop:function(){return this.is("started")&&(this.setState("started",!1),this.trigger("stop")),this}};return function(config){
var initConfig=_.defaults(config||{},_defaults);return makeTaskable(component(buttonApi)).on("finished",function(){this.stop()}).on("enqueued",function(){this.stop()}).setTemplate(buttonTpl).init(initConfig)}}),
