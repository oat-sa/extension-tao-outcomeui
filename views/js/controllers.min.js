...
iteration:3,interval:1e3}],pollAllIntervals:[{iteration:1,interval:8e3},{iteration:0,interval:5e3}]})}),define("taoOutcomeUi/plugins/results/action/view",["i18n","layout/actions/binder","core/plugin","util/url"],function(__,binder,pluginFactory,urlHelper){"use strict";return pluginFactory({name:"view",init:function(){var resultsList=this.getHost(),action={binding:"load",url:urlHelper.route("viewResult","Results","taoOutcomeUi")};resultsList.addAction({id:"view",label:__("View"),icon:"view",action:function(id){var context={id:id,classUri:resultsList.getClassUri()};binder.exec(action,context)}})}})}),define("taoOutcomeUi/plugins/results/action/delete",["jquery","i18n","uri","core/plugin","util/url","util/encode","ui/dialog/confirm","ui/feedback"],function($,__,uri,pluginFactory,urlHelper,encode,dialogConfirm,feedback){
"use strict";return pluginFactory({name:"delete",init:function(){var resultsList=this.getHost();resultsList.addAction({id:"delete",label:__("Delete"),icon:"delete",action:function(id){dialogConfirm(__("Please confirm deletion"),function(){$.ajax({url:urlHelper.route("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(id)},dataType:"json"}).done(function(response){response.deleted?(feedback().success(__("Result has been deleted")),resultsList.refresh()):(feedback().error(__("Something went wrong...")+"<br>"+encode.html(response.error),{encodeHtml:!1}),resultsList.trigger("error",response.error))}).fail(function(err){feedback().error(__("Something went wrong...")),
resultsList.trigger("error",err)})})}})}})}),define("taoOutcomeUi/plugins/results/action/download",["jquery","i18n","core/plugin","util/url","jquery.fileDownload"],function($,__,pluginFactory,urlHelper){"use strict";return pluginFactory({name:"download",init:function(){var resultsList=this.getHost();resultsList.addAction({id:"download",label:__("Download"),icon:"download",action:function(id){$.fileDownload(urlHelper.route("downloadXML","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"GET",data:{id:id,delivery:resultsList.getClassUri()
}})}})}})}),define("taoOutcomeUi/component/results/pluginsLoader",["core/pluginLoader","taoOutcomeUi/plugins/results/action/view","taoOutcomeUi/plugins/results/action/delete","taoOutcomeUi/plugins/results/action/download"],function(pluginLoader,actionView,actionDelete,actionDownload){"use strict";return pluginLoader({action:[actionView,actionDelete,actionDownload]})}),define("taoOutcomeUi/component/results/areaBroker",["lodash","core/areaBroker"],function(_,areaBroker){"use strict";var requireAreas=["list"];return _.partial(areaBroker,requireAreas)}),define("tpl!taoOutcomeUi/component/results/list",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="component results-list">\n    <div class="list"></div>\n</div>'})}),define("taoOutcomeUi/component/results/list",["jquery","i18n","lodash","uri","core/promise","ui/component","taoOutcomeUi/component/results/areaBroker","tpl!taoOutcomeUi/component/results/list","ui/datatable"],function($,__,_,uri,Promise,component,resultsAreaBroker,listTpl){"use strict";function resultsListFactory(config,pluginFactories){function pluginRun(method){var execStack=[];return _.forEach(plugins,function(plugin){_.isFunction(plugin[method])&&execStack.push(plugin[method]());
}),Promise.all(execStack)}var resultsList,areaBroker,classUri,plugins={},actions=[];if(!_.isPlainObject(config))throw new TypeError("The configuration is required");if(!_.isString(config.classUri)||!config.classUri)throw new TypeError("The class URI is required");if(!_.isString(config.dataUrl)||!config.dataUrl)throw new TypeError("The service URL is required");if(!_.isPlainObject(config.model)&&!_.isArray(config.model))throw new TypeError("The data model is required");return classUri=uri.decode(config.classUri),resultsList=component({refresh:function(){return areaBroker.getListArea().datatable("refresh"),this},addAction:function(name,action){var descriptor;return _.isPlainObject(name)?descriptor=name:_.isPlainObject(action)?(descriptor=action,
action.id=name):descriptor={id:name,label:name},_.isFunction(action)&&(descriptor.action=action),descriptor.label||(descriptor.label=descriptor.id),actions.push(descriptor),this},getConfig:function(){return this.config},getClassUri:function(){return classUri},getActions:function(){return actions}}),resultsList.before("render",function(){var self=this;return areaBroker=resultsAreaBroker(this.$component,{list:$(".list",this.$component)}),_.forEach(pluginFactories,function(pluginFactory){var plugin=pluginFactory(self,areaBroker);plugins[plugin.getName()]=plugin}),pluginRun("init").then(function(){return pluginRun("render")}).then(function(){areaBroker.getListArea().empty().on("query.datatable",function(){
self.setState("loading",!0).trigger("loading")}).on("load.datatable",function(){self.setState("loading",!1).trigger("loaded")}).datatable({url:self.config.dataUrl,model:self.config.model,actions:actions,filter:!0,labels:{filter:__("Search by delivery results")}})}).catch(function(err){return self.trigger("error",err),Promise.reject(err)})}).before("destroy",function(){var self=this;return pluginRun("destroy").then(function(){areaBroker.getListArea().empty()}).catch(function(err){self.trigger("error",err)})}).setTemplate(listTpl).init(config)}return resultsListFactory}),define("tpl!taoTaskQueue/component/button/tpl/report",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="task-report-container">\n    <div class="section-header flex-container-full">\n        <h2>',(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</h2>\n    </div>\n    <div class="main-container flex-container-full report"></div>\n</div>'})}),define("css!taoTaskQueue/component/button/css/taskable",[],function(){}),
define("taoTaskQueue/component/button/taskable",["jquery","lodash","i18n","core/promise","ui/report","ui/feedback","layout/loading-bar","tpl!taoTaskQueue/component/button/tpl/report","css!taoTaskQueue/component/button/css/taskable"],function($,_,__,Promise,reportFactory,feedback,loadingBar,reportTpl){"use strict";var defaultConfig={},taskableComponent={setTaskConfig:function(config){return _.assign(this.config,config),this},createTask:function(){var taskQueue,requestUrl,self=this,requestData={};return _.isFunction(this.config.taskCreationData)?requestData=this.config.taskCreationData.call(this):_.isPlainObject(this.config.taskCreationData)&&(requestData=this.config.taskCreationData),
this.config.taskCreationUrl?(requestUrl=this.config.taskCreationUrl,this.config.taskQueue?(taskQueue=this.config.taskQueue,loadingBar.start(),taskQueue.pollAllStop(),void taskQueue.create(requestUrl,requestData).then(function(result){var infoBox,message,task=result.task;result.finished?task.hasFile?taskQueue.download(task.id).then(function(){return taskQueue.archive(task.id)}).then(function(){self.trigger("finished",result),taskQueue.pollAll()}).catch(function(err){self.trigger("error",err),taskQueue.pollAll()}):taskQueue.archive(task.id).then(function(){self.trigger("finished",result),taskQueue.pollAll()}).catch(function(err){self.trigger("error",err),taskQueue.pollAll();
}):(message=__("<strong> %s </strong> has been moved to the background.",task.taskLabel),infoBox=feedback(null,{encodeHtml:!1,timeout:{info:8e3}}).info(message),taskQueue.trigger("taskcreated",{task:task,sourceDom:infoBox.getElement()}),self.trigger("enqueued",result)),loadingBar.stop()}).catch(function(err){taskQueue.pollAll(),self.trigger("error",err)})):this.trigger("error","the taskQueue model is required to create a task")):this.trigger("error","the request url is required to create a task")},displayReport:function(report,title,result){var $reportContainer,self=this;if(this.config.taskReportContainer instanceof $)return $reportContainer=$(reportTpl({title:title
})),this.config.taskReportContainer.html($reportContainer),reportFactory({actions:[{id:"continue",icon:"right",title:"continue",label:__("Continue")}]},report).on("action-continue",function(){self.trigger("continue",result)}).render($reportContainer.find(".report"))}};return function(component,config){return _.assign(component,taskableComponent),component.off(".taskable").on("init.taskable",function(){_.defaults(this.config,config||{},defaultConfig)})}}),define("tpl!taoTaskQueue/component/button/tpl/treeButton",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="tree-button-component">\n    <a class="li-inner">\n        <span class="spinner-icon icon-loop spinning"></span>\n        <span class="glyph start-icon icon-',(helper=helpers.icon)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.icon,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></span>\n        ',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{
hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"\n    </a>\n</div>"})}),define("css!taoTaskQueue/component/button/css/treeButton",[],function(){}),define("taoTaskQueue/component/button/treeButton",["jquery","lodash","i18n","ui/feedback","ui/component","layout/loading-bar","taoTaskQueue/component/button/taskable","tpl!taoTaskQueue/component/button/tpl/treeButton","css!taoTaskQueue/component/button/css/treeButton"],function($,_,__,feedback,component,loadingBar,makeTaskable,buttonTpl){"use strict";var _defaults={icon:"property-advanced",label:"OK"},buttonApi={start:function(){return this.createTask(),this.setState("started",!0),this.trigger("start"),
this},stop:function(){return this.is("started")&&(this.setState("started",!1),this.trigger("stop")),this}};return function(config){var initConfig=_.defaults(config||{},_defaults);return makeTaskable(component(buttonApi)).on("finished",function(){this.stop()}).on("enqueued",function(){this.stop()}).setTemplate(buttonTpl).init(initConfig)}}),define("taoOutcomeUi/controller/inspectResults",["jquery","lodash","i18n","module","core/logger","util/url","layout/actions/binder","layout/loading-bar","ui/feedback","taoTaskQueue/model/taskQueue","taoOutcomeUi/component/results/pluginsLoader","taoOutcomeUi/component/results/list","taoTaskQueue/component/button/treeButton"],function($,_,__,module,loggerFactory,urlHelper,binder,loadingBar,feedback,taskQueue,resultsPluginsLoader,resultsListFactory,treeTaskButtonFactory){
"use strict";function reportError(err){loadingBar.stop(),logger.error(err),err instanceof Error&&feedback().error(err.message)}var logger=loggerFactory("controller/inspectResults");return{start:function(){var taskButton,config=module.config()||{},$container=$("#inspect-result"),classUri=$container.data("uri"),listConfig={dataUrl:urlHelper.route("getResults","Results","taoOutcomeUi",{classUri:classUri}),model:config.dataModel,classUri:classUri};loadingBar.start(),_.forEach(config.plugins,function(plugin){plugin&&plugin.module&&(plugin.exclude?resultsPluginsLoader.remove(plugin.module):resultsPluginsLoader.add(plugin))}),$container.length?resultsPluginsLoader.load().then(function(){
...
uri:uniqueValue}}).start()})}}}),define("tpl!taoOutcomeUi/controller/resultModal",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="modal results-modal">\n\n</div>'})}),define("taoOutcomeUi/controller/resultsMonitoring",["jquery","i18n","util/url","uri","ui/feedback","util/locale","util/encode","layout/loading-bar","layout/actions/binder","ui/dialog/confirm","tpl!taoOutcomeUi/controller/resultModal","ui/datatable"],function($,__,url,uri,feedback,locale,encode,loadingBar,binder,dialogConfirm,resultModalTpl){
"use strict";function getRequestErrorMessage(xhr){loadingBar.start();var message="";try{var responseJSON=$.parseJSON(xhr.responseText);message=responseJSON.message?responseJSON.message:xhr.responseText}catch(e){message=xhr.responseText}return message}function viewResult(rowId){var res=parseRowId(rowId);loadingBar.start(),$.ajax({url:url.route("viewResult","Results","taoOutcomeUi",{id:res[0],classUri:res[1]}),type:"GET",success:function(result){var $container=$(resultModalTpl()).append(result);$resultsListContainer.append($container),$container.modal({startClosed:!0,minWidth:450}),$container.modal().css({"max-height":$(window).height()-80+"px","overflow-y":"auto"
}),$container.modal("open").on("closed.modal",function(){$container.modal("destroy"),$(this).remove()}),loadingBar.stop()},error:function(xhr,err){var message=getRequestErrorMessage(xhr);feedback().error(message,{encodeHtml:!1}),loadingBar.stop()}})}function checkValidItem(){return!this.start_time}function removeResult(rowId){var res=parseRowId(rowId);dialogConfirm(__("Please confirm deletion"),function(){$.ajax({url:url.route("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(res[0])},dataType:"json"}).done(function(response){response.deleted?(feedback().success(__("Result has been deleted")),$resultsListContainer.datatable("refresh")):(feedback().error(__("Something went wrong...")+"<br>"+encode.html(response.error),{
encodeHtml:!1}),$resultsListContainer.trigger("error",response.error))}).fail(function(err){feedback().error(__("Something went wrong...")),$resultsListContainer.trigger("error",err)})})}function downloadResult(rowId){var res=parseRowId(rowId);$.fileDownload(url.route("downloadXML","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"GET",data:{id:res[0],delivery:res[1]}})}function parseRowId(rowId){return rowId.split("|")}var $resultsListContainer=$(".results-list-container");return{start:function(){$resultsListContainer.datatable({
url:url.route("data","ResultsMonitoring","taoOutcomeUi"),filter:!0,labels:{filter:__("Search by results")},model:[{id:"delivery",label:__("Delivery"),sortable:!1},{id:"deliveryResultIdentifier",label:__("Delivery Execution"),sortable:!1},{id:"testTakerIdentifier",label:__("Test Taker"),sortable:!1},{id:"start_time",label:__("Start Time"),sortable:!1}],paginationStrategyTop:"simple",paginationStrategyBottom:"pages",rows:25,sortby:"result_id",sortorder:"desc",actions:{view:{id:"view",label:__("View"),icon:"view",action:viewResult,disabled:checkValidItem},remove:{id:"remove",title:__("Delete result"),icon:"bin",label:__("Delete"),action:removeResult,disabled:checkValidItem
},download:{id:"download",title:__("Download result"),icon:"download",label:__("Download"),action:downloadResult,disabled:checkValidItem}}})}}}),define("taoTaskQueue/component/button/standardButton",["jquery","lodash","i18n","core/promise","ui/report","ui/feedback","layout/loading-bar","ui/loadingButton/loadingButton","taoTaskQueue/component/button/taskable"],function($,_,__,Promise,reportFactory,feedback,loadingBar,loadingButton,makeTaskable){"use strict";var defaultConfig={};return function(config){var component;return config=_.defaults(config||{},defaultConfig),component=makeTaskable(loadingButton(config)),component.on("started",function(){this.createTask();
}).on("finished",function(){this.terminate().reset()}).on("enqueued",function(){this.terminate().reset()})}}),define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","module","util/url","ui/feedback","taoTaskQueue/model/taskQueue","taoTaskQueue/component/button/standardButton","ui/datatable"],function($,_,__,module,urlUtil,feedback,taskQueue,standardTaskButtonFactory){"use strict";var resulTableController={start:function(){var conf=module.config(),$container=$(".result-table"),$filterField=$(".result-filter",$container),$tableContainer=$(".result-table-container",$container),filter=conf.filter||"lastSubmitted",uri=conf.uri||"",columns=[],groups={},$actionBar=$container.find(".actions"),buildGrid=function(url,action,done){
$.ajax({url:url,dataType:"json",data:{filter:filter,uri:uri},type:"GET"}).done(function(response){response&&response.columns&&(columns="remove"===action?_.reject(columns,function(col){return _.find(response.columns,function(resCol){return _.isEqual(col,resCol)})}):"undefined"!=typeof response.first&&response.first===!0?response.columns.concat(columns):columns.concat(response.columns),_buildTable(done))})},_buildTable=function(done){var model=[];_.forEach(columns,function(col){model.push({id:col.prop?col.prop+"_"+col.contextType:col.contextId+"_"+col.variableIdentifier,label:col.label,sortable:!1})}),$tableContainer.empty().data("ui.datatable",null).off("load.datatable").on("load.datatable",function(){
_.isFunction(done)&&(done(),done="")}).datatable({url:urlUtil.route("feedDataTable","ResultTable","taoOutcomeUi",{filter:filter}),querytype:"POST",params:{columns:JSON.stringify(columns),_search:!1,uri:uri},model:model})};$("[data-group]",$container).each(function(){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action");groups[group]=groups[group]||{},groups[group][action]=$elt}),$container.on("click","[data-group]",function(e){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action"),url=$elt.data("url");e.preventDefault(),buildGrid(url,action,function(){_.forEach(groups[group],function($btn){$btn.toggleClass("hidden")})})}),buildGrid(urlUtil.route("getTestTakerColumns","ResultTable","taoOutcomeUi",{
filter:filter})),$filterField.select2({minimumResultsForSearch:-1}).select2("val",filter),$(".result-filter-btn",$container).click(function(){filter=$filterField.select2("val"),_buildTable()}),standardTaskButtonFactory({type:"info",icon:"export",title:__("Export CSV File"),label:__("Export CSV File"),taskQueue:taskQueue,taskCreationUrl:urlUtil.route("export","ResultTable","taoOutcomeUi"),taskCreationData:function(){return{filter:filter,columns:JSON.stringify(columns),uri:uri}}}).on("error",function(err){feedback().error(err)}).render($actionBar)}};return resulTableController}),define("json!taoItems/preview/resources/device-list.json",function(){return{tablets:{
"193986c3715c81838870f908fa98d69a":{label:"Amazon Kindle Fire HDX 7″",width:960,height:600},"4e8eafab11aad1486992d7ee2c8c16ca":{label:"Apple iPad",width:1024,height:768},"5aac2932d9ad00d6d8a1604b8f9e4e8d":{label:"Google Nexus 10, Motorola Xoom, Xyboard",width:1280,height:800},"9805d9753ad08c7630a9ee5418aa6c6c":{label:"Google Nexus 7",width:966,height:604},d4a431cedf4705d6bf2cba6d5788378b:{label:"Samsung Galaxy Tab 7.7, 8.9, 10.1",width:1280,height:800}},phones:{"1b89338531ef0155c8d6abc2ac02c81a":{label:"Apple iPhone 3GS",width:320,height:480,scaleFactor:1,dpWidth:320,dpHeight:480},"3f89da99d84214cde7df87ebe0699a6f":{label:"Apple iPhone 4",width:640,height:960,
scaleFactor:2,dpWidth:320,dpHeight:1920},"0d36971a5b98b367f54ced708c9af849":{label:"Apple iPhone 5",width:640,height:1136,scaleFactor:2,dpWidth:320,dpHeight:2272},"026beffcc051af995660ebdede986ace":{label:"BlackBerry Z10",width:768,height:1280,scaleFactor:2,dpWidth:384,dpHeight:2560},fd2dd53e667d6d49e4fa73356fa941dd:{label:"BlackBerry Z30",width:720,height:1280,scaleFactor:2,dpWidth:360,dpHeight:2560},"707584c1ae17d9b5b2cbe8603c91c147":{label:"Google Nexus 4",width:768,height:1280,scaleFactor:2,dpWidth:384,dpHeight:2560},"91823264de952e7f2347e07db5c4058b":{label:"Google Nexus 5",width:1080,height:1920,scaleFactor:3,dpWidth:360,dpHeight:5760},cd69817a6e147e8a4677389e56fcf568:{
label:"Google Nexus S",width:480,height:800,scaleFactor:1.5,dpWidth:320,dpHeight:1200},"75994faa6a38e31e99b53fcd75534a33":{label:"HTC Evo, Touch HD, Desire HD, Desire",width:480,height:800,scaleFactor:1.5,dpWidth:320,dpHeight:1200},"0cab4dfccdfc880687cb608cfe4159db":{label:"HTC One X, EVO LTE",width:720,height:1280,scaleFactor:2,dpWidth:360,dpHeight:2560},c4ff41fcba24d74fc21dc3490ca1edc9:{label:"HTC Sensation, Evo 3D",width:540,height:960,scaleFactor:1.5,dpWidth:360,dpHeight:1440},fa969a5e0dd2de53700eeee4ba4ba142:{label:"LG Optimus 2X, Optimus 3D, Optimus Black",width:480,height:800,scaleFactor:1.5,dpWidth:320,dpHeight:1200},"33c4f9364e295ec03531f3a3425819cf":{
label:"LG Optimus G",width:768,height:1280,scaleFactor:2,dpWidth:384,dpHeight:2560},bc47f5275f0efd56fee52d43a8082981:{label:"LG Optimus LTE, Optimus 4X HD",width:720,height:1280,scaleFactor:1.7,dpWidth:424,dpHeight:2176},"0732887e6e324fd10777b735520a34cf":{label:"LG Optimus One",width:320,height:480,scaleFactor:1.5,dpWidth:213,dpHeight:720},"2e38a78364f09d0da0e9e1a3a68e7fb0":{label:"Motorola Defy, Droid, Droid X, Milestone",width:480,height:854,scaleFactor:1.5,dpWidth:320,dpHeight:1281},bd4b78dddc7c9625a4141a1b1ddabb87:{label:"Motorola Droid 3, Droid 4, Droid Razr, Atrix 4G, Atrix 2",width:540,height:960,scaleFactor:1,dpWidth:540,dpHeight:960},"3bf1edf21cfa81006183d2b02974c84e":{
...
return buffer+='<div class="preview-overlay tao-scope overlay preview-',(helper=helpers.previewType)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.previewType,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+' item-no-print"\n     style="display:none">\n    <div class="preview-container-outer">\n        <form class="preview-utility-bar plain">\n            <div class="preview-utility-bar-inner grid-row">\n\n                <div class="col-4">\n                    <input type="hidden" value="" data-value="" class="standard-device-selector preview-device-selector" data-target="standard">\n                    <select class="preview-type-selector">\n                        ',
