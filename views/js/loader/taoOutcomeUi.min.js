...
taskCreationData:{uri:uniqueValue}}).start()})}}}),define("tpl!taoOutcomeUi/controller/resultModal",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="modal results-modal">\n\n</div>'})}),define("taoOutcomeUi/controller/resultsMonitoring",["jquery","i18n","util/url","uri","ui/feedback","util/locale","util/encode","layout/loading-bar","layout/actions/binder","ui/dialog/confirm","tpl!taoOutcomeUi/controller/resultModal","ui/datatable"],function($,__,url,uri,feedback,locale,encode,loadingBar,binder,
dialogConfirm,resultModalTpl){"use strict";var $resultsListContainer=$(".results-list-container");function viewResult(rowId){var res=parseRowId(rowId);loadingBar.start(),$.ajax({url:url.route("viewResult","Results","taoOutcomeUi",{id:res[0],classUri:res[1]}),type:"GET",success:function(result){var $container=$(resultModalTpl()).append(result);$resultsListContainer.append($container),$container.modal({startClosed:!0,minWidth:450}),$container.modal().css({"max-height":$(window).height()-80+"px",overflow:"auto"}),$container.on("click",function(e){var $modalContainer=$(e.target).parents(".modal");$(e.target).hasClass("preview")&&(
$modalContainer.length&&$modalContainer.trigger("closed.modal"),$(".preview-overlay").css({zIndex:$container.modal().css("z-index")+1}))}),$container.modal("open").on("closed.modal",function(){$container.modal("destroy"),$(".modal-bg").remove(),$(this).remove()}),loadingBar.stop()},error:function(xhr,err){var message=function(xhr){loadingBar.start();var message="";try{var responseJSON=$.parseJSON(xhr.responseText);message=responseJSON.message?responseJSON.message:xhr.responseText}catch(e){message=xhr.responseText}return message}(xhr);feedback().error(message,{encodeHtml:!1}),loadingBar.stop()}})}function checkValidItem(){return!this.start_time}
function downloadResult(rowId){var res=parseRowId(rowId);$.fileDownload(url.route("downloadXML","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"GET",data:{id:res[0],delivery:res[1]}})}function parseRowId(rowId){return rowId.split("|")}return{start:function(){$resultsListContainer.datatable({url:url.route("data","ResultsMonitoring","taoOutcomeUi"),filter:!0,labels:{filter:__("Search by results")},model:[{id:"delivery",label:__("Delivery"),sortable:!1},{id:"deliveryResultIdentifier",label:__("Delivery Execution"),
sortable:!1},{id:"testTakerIdentifier",label:__("Test Taker"),sortable:!1},{id:"start_time",label:__("Start Time"),sortable:!1}],paginationStrategyTop:"simple",paginationStrategyBottom:"pages",rows:25,sortby:"result_id",sortorder:"desc",actions:{view:{id:"view",label:__("View"),icon:"external",action:viewResult,disabled:checkValidItem},download:{id:"download",title:__("Download result"),icon:"download",label:__("Download"),action:downloadResult,disabled:checkValidItem}}})}}}),define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","module","util/url","ui/feedback","core/taskQueue/taskQueue","ui/taskQueueButton/standardButton","ui/datatable"],
...
return _.isEqual(col,resCol)})}):void 0!==response.first&&!0===response.first?response.columns.concat(columns):columns.concat(response.columns),_buildTable(done))})},_buildTable=function(done){var model=[];_.forEach(columns,function(col){model.push({id:col.prop?col.prop+"_"+col.contextType:col.contextId+"_"+col.variableIdentifier,label:col.label,sortable:!1})}),$tableContainer.empty().data("ui.datatable",null).off("load.datatable").on("load.datatable",function(){_.isFunction(done)&&(done(),done="")}).datatable({url:urlUtil.route("feedDataTable","ResultTable","taoOutcomeUi",{filter:filter}),querytype:"POST",params:{columns:JSON.stringify(columns),_search:!1,
uri:uri},model:model})};$("[data-group]",$container).each(function(){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action");groups[group]=groups[group]||{},groups[group][action]=$elt}),$container.on("click","[data-group]",function(e){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action"),url=$elt.data("url");e.preventDefault(),buildGrid(url,action,function(){_.forEach(groups[group],function($btn){$btn.toggleClass("hidden")})})}),buildGrid(urlUtil.route("getTestTakerColumns","ResultTable","taoOutcomeUi",{filter:filter})),$filterField.select2({minimumResultsForSearch:-1}).select2("val",filter),$(".result-filter-btn",$container
).click(function(){filter=$filterField.select2("val"),_buildTable()}),standardTaskButtonFactory({type:"info",icon:"export",title:__("Export CSV File"),label:__("Export CSV File"),taskQueue:taskQueue,taskCreationUrl:urlUtil.route("export","ResultTable","taoOutcomeUi"),taskCreationData:function(){return{filter:filter,columns:JSON.stringify(columns),uri:uri}}}).on("error",function(err){feedback().error(err)}).render($actionBar)}}}),define("taoOutcomeUi/controller/routes",[],function(){"use strict";return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}},
ResultsMonitoring:{actions:{index:"controller/resultsMonitoring"}}}}),define("taoOutcomeUi/controller/viewResult",["module","jquery","i18n","util/url","layout/section","taoItems/previewer/factory","jquery.fileDownload"],function(module,$,__,urlHelper,section,previewerFactory){"use strict";return{start:function(){var conf=module.config(),$container=$("#view-result"),$resultFilterField=$(".result-filter",$container),$classFilterField=$('[name="class-filter"]',$container),classFilter=JSON.parse(conf.filterTypes)||[];for(var i in $resultFilterField.select2({minimumResultsForSearch:-1}).select2("val",conf.filterSubmission||"all"),classFilter)$(
'[value="'+classFilter[i]+'"]').prop("checked","checked");$(".result-filter-btn",$container).click(function(e){classFilter=[""],$classFilterField.each(function(){$(this).prop("checked")&&classFilter.push($(this).val())}),section.loadContentBlock(urlHelper.route("viewResult","Results","taoOutcomeUi"),{id:conf.id,classUri:conf.classUri,filterSubmission:$resultFilterField.select2("val"),filterTypes:classFilter})}),$(".download",$container).on("click",function(e){var variableUri=$(this).val();$.fileDownload(urlHelper.route("getFile","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__(
"There was a problem generating your report, please try again."),httpMethod:"POST",data:{variableUri:variableUri,deliveryUri:conf.classUri}})}),$(".preview",$container).on("click",function(e){var $this=$(this),deliveryId=$this.data("deliveryId"),resultId=$this.data("resultId"),itemDefinition=$this.data("definition"),uri=$this.data("uri"),type=$this.data("type"),state=$this.data("state");e.preventDefault(),deliveryId&&resultId&&itemDefinition&&(uri={uri:uri,resultId:resultId,itemDefinition:itemDefinition,deliveryUri:deliveryId}),previewerFactory(type,uri,state,{readOnly:!0,fullPage:!0})})}}}),define("taoOutcomeUi/loader/taoOutcomeUi.bundle",function(){}),
window.bundles=(window.bundles||[]).concat(["taoItems/loader/taoItems.min"]),window.loadBundles&&window.loadBundles();
